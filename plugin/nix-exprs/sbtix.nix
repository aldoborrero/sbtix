...
with lib; let
  sbtTemplate = repoDefs: versioning: let
  ...
  mergeSbtTemplates = templates:
    runCommand "merge-sbt-template" {}
    (
      let
...
in rec {
  mkRepo = name: artifacts:
    runCommand name {}
    (let
      parentDirs = filePath:
        concatStringsSep "/" (init (splitString "/" filePath));
      linkArtifact = outputPath: urlAttrs: let
  ...
  repoConfig = {
    ...
    repoPath = repoName: repoPattern: ["${name}-${repoName}: file://${nixrepo}/${repoName}${repoPatternOptional repoPattern}"];
  in
